...
void dowork() {  //串行消元--消元子->被消元行
	...
	for (i = col - 1; i >= 0; i--) {
		if (!eliminer[i].isnull()) {
			for (j = rank; j < elinenum; j+=mpisize) {
				if (eline[j].mycol == i)
			...
			int t = -1;
			if (rank != 0) {
				for (k = rank; k <elinenum; k+=mpisize)
					if (eline[k].mycol == i) {
						pass[rank] = eline[k];
						t = k;
				...
				for (k = mpisize - 1; k > 0; k--) {
					...
					if (t != -1&&(isupgrade==-1||t<isupgrade)) {
						eliminer[i] = pass[k];
						isupgrade = t;
				...
				for (k = rank; k <elinenum; k+=mpisize)
					if (eline[k].mycol == i&&k<isupgrade) {
						eliminer[i] = eline[k];
						isupgrade = k;
						break;
					}
	...
	if (rank != 0)
		...
		for (k = 1; k < mpisize; k++) {
			for (int q = k; q <elinenum; q+=mpisize) {
